namespace RazorEngine.Compilation
{
    ...
    public abstract class DirectCompilerServiceBase : CompilerServiceBase, IDisposable
    {
        ...
        private Tuple<CompilerResults, string> Compile(TypeContext context)
        {
            ...
            if (Debug)
            {
                ...
                using (var writer = new StringWriter(builder))
                {
                    ...
                    //TODO: this is hack so we can get the correct generic model.
                    sourceCode = sourceCode.Replace(": RazorEngine.Templating.TemplateBase",
                                                ": RazorEngine.Templating.TemplateBase<" + context.ReplacementViewModelName + ">");

                    //and correct class name.
                    sourceCode = sourceCode.Replace(context.ClassName, context.ReplacementClassName);

                    //and namespace.
                    sourceCode = sourceCode.Replace("namespace CompiledRazorTemplates.Dynamic", "namespace " + context.ReplacementNamespace);
                }

                return Tuple.Create(new CompilerResults(new TempFileCollection()), sourceCode);
            }
        ...
        /// <summary>
        /// Compiles the type defined in the specified type context and return the source used in compilation.
        /// </summary>
        /// <param name="context">The type context which defines the type to compile.</param>
        /// <returns>The source of compiled type.</returns>
        [Pure, SecurityCritical]
